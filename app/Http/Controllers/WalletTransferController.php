...
public function lockDeposit(Request $request){
    $validator = Validator::make($request->all(),[
        'honeypotu' => ['required','gt:'.\Session::get('logtime')],
        'amount' => ['required','numeric'],
        'password' => ['required','string'],
    ...
    try{
        ...
        // Insert Locking Stacking Deposit
        \App\StackingDeposite::insert([
            'userid' => $userUpdate->id,
            'txnid' => $insWalletEntry,
            'amount' => $request->amount/$price->price,
            'usdt' => $request->amount,
            'planid' => 0, // agar koi plan nahi to 0
            'status' => 1,
            'istatus' => 1,
        ...
        \DB::commit();
        return redirect('/User/Lock')->with('success','Your locking deposit is submitted successfully.');
