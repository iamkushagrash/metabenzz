...
class AssetDetailController extends Controller
{
    ...
    public function userDirectTeamList(Request $request){
        ...
        ->select('u.id as id', 'u.usersname as name', 'u.email as email', 'u.uuid as userid', 'u.doj as doj', 'ud.current_self_investment as shares','ud.total_investment as teamtotal', 'u.wallet_address as bep20address')
        ...
        if(is_null($udata)){
            return view('control.userall')->with('Warning','This User Does Not Exists.');
                ...
                for($i=1;$i<1000;$i++){
                        ...
                        ->select('users.id as id','users.usersname as name','users.email as email','users.uuid as userid','ud.current_self_investment as current', 'users.wallet_address as bep20address')
